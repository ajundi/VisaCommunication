...
namespace OpenTapVisa
{
    public static class Visa
    {
        ...
        public static int viRead(int vi, out string asciiResults)
        {
            asciiResults = string.Empty;
            int retCount = 0;
            int buffersize = 1024;
            byte[] byteResults = new byte[buffersize];
            
            int retVal = viRead(vi, byteResults,byteResults.Length , out retCount);
            while (retVal == VI_SUCCESS_MAX_CNT)
            {
                byte[] byteResults_Temp = new byte[buffersize];
                retVal = viRead(vi, byteResults_Temp, byteResults_Temp.Length, out int retCount_temp);
                retCount = retCount_temp + retCount;
                byteResults =Combine(byteResults, byteResults_Temp);
            }
            if (retVal >= Visa.VI_SUCCESS)
            {
                ASCIIEncoding encoding = new ASCIIEncoding();
                while (byteResults[retCount-1] == 10 || byteResults[retCount - 1] == 13) { retCount -= 1; } // reduce count by one to remove \n or \r from end of string
                asciiResults = encoding.GetString(byteResults, 0, retCount);
            }
            return retVal;
        }
        
        public static int IOWrite(int vi, string command, out int retCount)
        {
            int status = viWrite(vi, Encoding.ASCII.GetBytes(command), command.Length, out retCount);
            return status;
        }

        public static int IOQuery(int vi, string command, out string ReadBuffer)
        {
            int status=viWrite(vi, Encoding.ASCII.GetBytes(command), command.Length, out int retCountbuffer);
            if (status == 0)
            {
                status = viRead(vi, out ReadBuffer);
    ...
    static Visa()
    {
        var IsWin32 = OpenTap.OperatingSystem.Current == OpenTap.OperatingSystem.Windows;
    ...
    public static int viOpenDefaultRM(out int sesn) { return viOpenDefaultRMRef(out sesn); }

    public static int viFindRsrc(int sesn, string expr, out int vi, out int retCount, StringBuilder desc) { return viFindRsrcRef(sesn, expr, out vi, out retCount, desc); }
    public static int viFindNext(int vi, StringBuilder desc) { return viFindNextRef(vi, desc); }

    public static int viParseRsrc(int sesn, string desc, ref short intfType, ref short intfNum) { return viParseRsrcRef(sesn, desc, ref intfType, ref intfNum); }
    public static int viParseRsrcEx(int sesn, string desc, ref short intfType, ref short intfNum, StringBuilder rsrcClass, StringBuilder expandedUnaliasedName, StringBuilder aliasIfExists) { return viParseRsrcExRef(sesn, desc, ref intfType, ref intfNum, rsrcClass, expandedUnaliasedName, aliasIfExists); }

    public static int viOpen(int sesn, string viDesc, int mode, int timeout, out int vi) { return viOpenRef(sesn, viDesc, mode, timeout, out vi); }
    public static int viClose(int vi) { return viCloseRef(vi); }

    public static int viGetAttribute(int vi, int attrName, out byte attrValue) { return viGetAttribute1Ref(vi, attrName, out attrValue); }
    public static int viGetAttribute(int vi, int attrName, StringBuilder attrValue) { return viGetAttribute2Ref(vi, attrName, attrValue); }
    public static int viGetAttribute(int vi, int attrName, out int attrValue) { return viGetAttribute3Ref(vi, attrName, out attrValue); }
    public static int viSetAttribute(int vi, int attrName, byte attrValue) { return viSetAttribute1Ref(vi, attrName, attrValue); }
    public static int viSetAttribute(int vi, int attrName, int attrValue) { return viSetAttribute2Ref(vi, attrName, attrValue); }

    public static int viStatusDesc(int vi, int status, StringBuilder desc) { return viStatusDescRef(vi, status, desc); }

    public static int viEnableEvent(int vi, int eventType, short mechanism, int context) { return viEnableEventRef(vi, eventType, mechanism, context); }
    public static int viDisableEvent(int vi, int eventType, short mechanism) { return viDisableEventRef(vi, eventType, mechanism); }
    public static int viInstallHandler(int vi, int eventType, viEventHandler handler, int UserHandle) { return viInstallHandlerRef(vi, eventType, handler, UserHandle); }
    public static int viUninstallHandler(int vi, int eventType, viEventHandler handler, int userHandle) { return viUninstallHandlerRef(vi, eventType, handler, userHandle); }
    ...
    public static int viReadSTB(int vi, ref short status) { return viReadSTBRef(vi, ref status); }

    public static int viClear(int vi) { return viClearRef(vi); }

    public static int viLock(int vi, int lockType, int timeout, string requestedKey, StringBuilder accessKey) { return viLockRef(vi, lockType, timeout, requestedKey, accessKey); }
    public static int viUnlock(int vi) { return viUnlockRef(vi); }
